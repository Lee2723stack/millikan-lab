...
async function start() {
  ...
  app.get('/health', (req, res) => {
    try { getDb(); return res.json({ status: 'ok', db: true }); }
    catch(e) { return res.json({ status: 'ok', db: false }); }
  });
  ...
  app.use((err, req, res, next) => {
    if (err.message && err.message.includes('仅支持')) return res.status(400).json({ error: err.message });
